package net.ben.neatmobsmod.entity.custom;

import net.ben.neatmobsmod.NEATMobsMod;
import net.ben.neatmobsmod.entity.ModEntities;
import net.ben.neatmobsmod.entity.ai.goal.EvaluateOutputs;
    ...
    public void tick() {
        ...
        if (this.world.isClient == false&&this.isAlive()){
            ...
            if ((ticksSurvived%2)==0 && genomeNum >= 0) {
                NEATMobsMod.zomboNEAT.getGenomeOutput(this);
            }
        }
        super.tick();
    }
    ...
    private float getAverageZomboYaw (){
        ...
        for (Entity entity:nearby){
            if (entity.getType() == ModEntities.ZOMBO){
                totalZombos ++;
                totalYaw += ((ZomboEntity) entity).getAllYaw() +180;
            }
        }
